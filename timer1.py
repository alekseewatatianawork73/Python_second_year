...
# функция для отсчёта времени (таймер)
def timer(entry, btn1_2):
    global second
    k = int(entry.get()) + 1  # начальное количество секунд, которое задал пользователь
    # установили значение на единицу больше, чтобы цикл не закончился раньше времени
    second = 0  # текущее количество секунд
    while k:
        btn1_2.configure(text=f"Секунд прошло: {second}")  # меняем надпись
